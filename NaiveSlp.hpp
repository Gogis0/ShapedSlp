...
public:
    ...
    std::vector<uint64_t> base_powers_; // hash for the pattern suffixes
...
public:
    ...
    {
      for (uint64_t i = 0; i < getNumRules(); ++i) {
          //std::cout << rules_[i].left << " " << rules_[i].right << std::endl;
          const uint64_t l_hash = (rules_[i].left < getAlphSize()) ?
                                  hash_char(rules_[i].left) : hashVec[rules_[i].left - getAlphSize()];
